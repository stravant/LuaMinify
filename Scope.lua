...
function Scope.GetOldLocal(self, name)
...
function Scope.MapLocal(self, name, var)
	self.oldLocalNamesMap[name] = var
end

function Scope.GetOldGlobal(self, name)
...
function Scope.MapGlobal(self, name, var)
	self.oldGlobalNamesMap[name] = var
end

function Scope.GetOldVariable(self, name)
	return self:GetOldLocal(name) or self:GetOldGlobal(name)
end

function Scope.RenameLocal(self, oldName, newName)
...
function Scope.RenameGlobal(self, oldName, newName)
	oldName = type(oldName) == 'string' and oldName or oldName.Name
	local found = false
...
function Scope.RenameVariable(self, oldName, newName)
	oldName = type(oldName) == 'string' and oldName or oldName.Name
...
function Scope.GetAllVariables(self)
...
function Scope.GetVars(self, top)
	local ret = { }
	if top then
...
function Scope.GetVariable(self, name)
	return self:GetLocal(name) or self:GetGlobal(name)
end

function Scope.ObfuscateLocals(self, recommendedMaxLength, validNameChars)
